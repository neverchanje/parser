...
static inline std::vector<LAItem> itemSetToVector(const LAItemSet &I) {
  return std::vector<LAItem>(I.begin(), I.end());
}

struct LAItemSetHasher {
  size_t operator()(const LAItemSet &val) const {
    return boost::hash_value(val);
  }
};

typedef std::unordered_map<LAItemSet, DFA::State, LAItemSetHasher> StateTable;
...
insertInTable(StateTable &table, LAItemSet &&iset, DFA::State &&state) {
...
Automaton Automaton::Make(RuleID init) {
  ...
  ret = insertInTable(table, {LAItem(init, 0)}, DFA::State(DFA::START_STATE));
  que.push(ret.first);

  while (!que.empty()) {
    const auto &I = QFRONT_ISET(que); // I contains only kernel items.
    clsr = LAClosure(itemSetToVector(I));
//    DumpClosure(clsr);
    tmp.clear();
    ...
    for (auto i = clsr.begin(); i != clsr.end(); i++) {
      ...
      if (!i->AtEnd()) {
        tmp.insert(LAItem(i->rule_id, i->offset + 1));
      }
